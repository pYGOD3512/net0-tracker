...
const StatisticWidget = ({ title, value, status, status_percentage, subtitle, prefix }) => {
	return (
		<Card>
			...
			<div className={`${prefix? 'd-flex': ''} ${title ? 'mt-3':''}`}>
				...
				<div>
					<div className="d-flex align-items-center">
						...
						{
							status ? 
							<span className={`font-size-base font-weight-bold ml-1 ${status !== 0 && status > 0 ? 'text-success' : 'text-danger'}`} >
								{status}
								<span className='ml-1 mr-1'>{ status_percentage}</span>
